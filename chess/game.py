...
class Game:
    def __init__(self, gap_white: int, gap_black: int):
        ...
        self.board[1][gap_white] = EMPTY

        for i in range(0, 8):
            self.board[6][i] = BLACK

        self.board[6][gap_black] = EMPTY
    ...
    def unapply_move(self):
        ...
        if len(self.moves) > 0 and self.moves[-1].is_double_push():
            self.enpassant_col = self.moves[-1].to[1]
        else:
            self.enpassant_col = -1
    ...
    def check_terminal(self, moves: List[Move]) -> Tuple[bool, int]:
        if len(moves) == 0:
            return True, EMPTY
        for i in range(0, 8):
